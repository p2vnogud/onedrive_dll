...
extern "C" __declspec(dllexport) void ReflectiveLoader()
{
    ...
    DWORD64 dll_image_address = get_current_address();
    ...
    char KERNEL32_DLL_string[] = { '\x4b', '\x45', '\x52', '\x4e', '\x45', '\x4c', '\x33', '\x32', '\x2e', '\x44', '\x4c', '\x4c', 0 };
    char VirtualAlloc_string[] = { '\x56', '\x69', '\x72', '\x74', '\x75', '\x61', '\x6c', '\x41', '\x6c', '\x6c', '\x6f', '\x63', 0 };
    char GetProcAddress_string[] = { '\x47', '\x65', '\x74', '\x50', '\x72', '\x6f', '\x63', '\x41', '\x64', '\x64', '\x72', '\x65', '\x73', '\x73', 0 };
    char LoadLibraryA_string[] = { '\x4c', '\x6f', '\x61', '\x64', '\x4c', '\x69', '\x62', '\x72', '\x61', '\x72', '\x79', '\x41', 0 };
    char USER32_DLL_string[] = { '\x75', '\x73', '\x65', '\x72', '\x33', '\x32', '\x2e', '\x44', '\x4c', '\x4c', 0 };
    char MessageBoxA_string[] = { '\x4d', '\x65', '\x73', '\x73', '\x61', '\x67', '\x65', '\x42', '\x6f', '\x78', '\x41', 0 };
    ...
    // Tải user32.dll và MessageBoxA
    HMODULE user32 = pLoadLibraryA(USER32_DLL_string);
    if (!user32) return;

    //pMessageBoxA = (MESSAGEBOXA)pGetProcAddress(user32, MessageBoxA_string);
    //if (!pMessageBoxA) return;

    //// Thử gọi MessageBoxA để debug
    //pMessageBoxA(NULL, "Reached ReflectiveLoader", "Debug", MB_OK);
    ...
    if (!dll_base) {
        pMessageBoxA(NULL, "Failed to allocate memory", "Error", MB_OK);
        return;
    }
    ...
    // 4. copy sections
    DWORD section_count = nt_headers_address->FileHeader.NumberOfSections;
    PIMAGE_SECTION_HEADER section_header_address = IMAGE_FIRST_SECTION(nt_headers_address);
    for (DWORD i = 0; i < section_count; i++, section_header_address++)
    {
        DWORD64 section_virtual_address = dll_base + section_header_address->VirtualAddress;
        DWORD64 section_data_address = dll_image_address + section_header_address->PointerToRawData;
    ...
    if (imports_data_directory.Size)
    {
        ...
        {
            LPCSTR library_name = (LPCSTR)(dll_base + import_descriptor->Name);
            HMODULE library_address = pLoadLibraryA(library_name);
            if (!library_address) {
                pMessageBoxA(NULL, "Failed to load library", "Error", MB_OK);
                return;
            }
            PIMAGE_THUNK_DATA thunk_data_address = (PIMAGE_THUNK_DATA)(dll_base + import_descriptor->FirstThunk);
            for (; thunk_data_address->u1.AddressOfData != (DWORD64)NULL; thunk_data_address++)
            {
                PIMAGE_IMPORT_BY_NAME import_by_name_address = (PIMAGE_IMPORT_BY_NAME)(dll_base + thunk_data_address->u1.AddressOfData);
                thunk_data_address->u1.Function = (DWORD64)pGetProcAddress(library_address, import_by_name_address->Name);
                if (!thunk_data_address->u1.Function) {
                    pMessageBoxA(NULL, "Failed to resolve function", "Error", MB_OK);
                    return;
                }
            }
        }
    ...
    DWORD64 base_diff = dll_base - nt_headers_address->OptionalHeader.ImageBase;
    ...
    {
        PIMAGE_BASE_RELOCATION base_reloc_address = (PIMAGE_BASE_RELOCATION)(dll_base + reloc_data_directory.VirtualAddress);
        while (base_reloc_address->SizeOfBlock)
        {
            DWORD64 reloc_block_address = dll_base + base_reloc_address->VirtualAddress;
            DWORD64 reloc_block_entry_count = (base_reloc_address->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / sizeof(BASE_RELOCATION_ENTRY);
            PBASE_RELOCATION_ENTRY reloc_entry_address = (PBASE_RELOCATION_ENTRY)((DWORD64)base_reloc_address + sizeof(IMAGE_BASE_RELOCATION));
    ...
    // 7. call entry point
    pMessageBoxA(NULL, "Reached before calling DllMain", "Debug", MB_OK);
...
{
    if (pMessageBoxA) {
        pMessageBoxA(NULL, "Hello from DllMain!", "Reflective Dll Injection", MB_OK);
    }
    else {
